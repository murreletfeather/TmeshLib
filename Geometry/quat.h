...
public:
    ...
    */
    ...
    /*!
     *  计算四元数共轭：四元数 q = (w, x, y, z)，共轭就是 q* = (w, -x, -y, -z)
     *  \return 共轭后的新四元数
     *
     *  对于单位四元数（表示旋转），共轭等于逆，直接用来表示逆旋转
     */
    CQuat conjugate() const
    {
        return CQuat(m_w, -m_x, -m_y, -m_z);
    }

    /*!
     *  求四元数的逆，对于归一化后的单位四元数（旋转）直接就是共轭，非常高效
     *  \return 逆四元数
     *
     *  说明：本函数假设当前已经是单位四元数（归一化），如果不是非单位四元数需要更通用求逆，可后续扩展
     *  旋转场景下所有使用的四元数都是归一化单位四元数，所以这个快速实现足够
     */
    CQuat inverse() const
    {
        return conjugate();
